...
def set_minus(set11,set22):
    print(set11.difference(set22))
    
def is_member_of_set(setB):
    print(input("Enter the the element to check: ") in setB)    

def set_display(common_set):
    print(len(common_set))
    
def set_add_element(setA):
    setA.add(input("Enter the element: "))
    print(setA)

def set_add_elements(setA):
    setA.update(set(input("Enter the elements: ").split(",")))   
    print(setA)
    
def set_remove_element(setA):
    print(setA)
    setA.remove(input("Enter the element: ")) #throws error
    # setA.discard(input("Enter the element: ")) #normal execution continued
    print(setA)
